...
def init_fusion_score_model_ovi(rank: int = 0, meta_init=False):
    # Use absolute path relative to this file's parent directory (ovi/)
    _utils_dir = os.path.dirname(os.path.abspath(__file__))
    _ovi_dir = os.path.dirname(_utils_dir)  # Go up to ovi/ directory
    video_config_path = os.path.join(_ovi_dir, "configs", "model", "dit", "video.json")
    audio_config_path = os.path.join(_ovi_dir, "configs", "model", "dit", "audio.json")
    
    assert os.path.exists(video_config_path), f"{video_config_path} does not exist"
    assert os.path.exists(audio_config_path), f"{audio_config_path} does not exist"
