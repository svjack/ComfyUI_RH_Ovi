...
_config_path = os.path.join(_current_file_dir, 'configs', 'inference', 'inference_fusion.yaml')
    ...
    def generate(self,
        ...
        try:
            ...
            with torch.amp.autocast('cuda', enabled=self.target_dtype != torch.float32, dtype=self.target_dtype):
                timestep_iterator = enumerate(zip(timesteps_video, timesteps_audio))
                # Use tqdm only if no progress callback is provided
                if progress_callback is None:
                    timestep_iterator = tqdm(timestep_iterator)
                
                for i, (t_v, t_a) in timestep_iterator:
